module AgentSpace::NewAgentManager {
    ...
    const E_NOT_INITIALIZED: u64 = 1000;
    const E_ALREADY_INITIALIZED: u64 = 1001;

    public entry fun initialize(account: &signer) {
        let account_addr = signer::address_of(account);
        assert!(!exists<AgentManagerData>(account_addr), E_ALREADY_INITIALIZED);
        
        if (!AgentRegistry::is_initialized(account_addr)) {
    ...
    public entry fun create_predefined_agents(creator: &signer) acquires AgentManagerData {
        ...
        assert!(exists<AgentManagerData>(creator_address), E_NOT_INITIALIZED);
